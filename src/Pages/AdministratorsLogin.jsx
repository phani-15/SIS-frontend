...
import { Eye, EyeOff, KeyRound, ArrowRight, ShieldCheck, Sparkles, UserCog } from 'lucide-react';
import { Administrators } from '../assets/Data';

const AdministratorsLogin = () => {
      ...
      <div className="w-full max-w-6xl overflow-hidden rounded-[32px] sis-panel grid lg:grid-cols-[1.05fr_0.95fr]">
        <div className="p-8 lg:p-10">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-[var(--primary)]">Administrator sign-in</h1>
              <p className="text-sm text-[var(--on-surface-variant)]">Access coordinator and leadership tools in one secure experience.</p>
            </div>
          </div>

          <div className="mb-5 flex flex-wrap items-center gap-2">
            <span className="sis-accent-pill"><Sparkles size={15} /> Guided access</span>
            <span className="sis-accent-pill"><ShieldCheck size={15} /> Encrypted login</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-[var(--on-surface)]">Role</label>
              <select name="role" id="role" value={formData.role} onChange={handleInputChange} className="w-full rounded-xl border border-[var(--outline-variant)] bg-[var(--surface-container-lowest)] px-3 py-3 text-sm outline-none transition focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/10">
        ...
        <div className="relative hidden lg:block">
          <img src="/images/overview.webp" alt="" className="h-full w-full object-cover" />
          ...
          <div className="absolute bottom-8 left-8 right-8 rounded-2xl border border-white/20 bg-white/15 p-5 text-white backdrop-blur-md">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[var(--primary-fixed)]">Leadership portal</p>
            <h2 className="mt-2 text-2xl font-semibold">Stay aligned with student progress and academic priorities.</h2>
          </div>
        </div>
      </div>
    </div>
...
export default AdministratorsLogin;