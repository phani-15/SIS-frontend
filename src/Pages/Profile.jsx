...
export default function Profile() {
  ...
  const [studentData, setStudentData] = useState({
    ...
    academicHistory: [
      { institution: 'JNTU-GV Vizianagaram', degree: 'Bachelor of Technology', yearOfCompletion: 2027, grade: 'Pursuing' },
      { institution: 'Sri Chaitanya School', degree: 'Intermediate (12th)', yearOfCompletion: 2021, grade: 'A+' },
      { institution: 'Sri Chaitanya School', degree: 'SSC (10th)', yearOfCompletion: 2019, grade: 'A+' }
    ],
    skills: ['Python', 'Java', 'Web Development', 'Data Structures', 'Database Design', 'Cloud Computing'],
    certifications: [
      { name: 'AWS Certified Cloud Practitioner', issuer: 'Amazon Web Services', date: '2024-03-15' },
      { name: 'Google Cloud Associate Cloud Engineer', issuer: 'Google Cloud', date: '2024-01-20' }
    ],
    projects: [
      { title: 'Student Information System', description: 'Centralized platform for managing student data', year: 2024 },
      { title: 'E-Commerce Platform', description: 'Full-stack web application with payment integration', year: 2023 }
    ]
  });
          ...
          <div className={`relative rounded-[24px] overflow-hidden border border-[#8A2E88]/22 mb-8
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-8 p-8 lg:p-10">
              ...
              <div className="flex flex-col items-center gap-4 md:w-1/4">
                <div className="icon-pulse w-[120px] h-[120px] rounded-[20px] flex-shrink-0
                  bg-gradient-to-br from-[#8A2E88] to-[#C084C8]
                  flex items-center justify-center"
                  style={{ boxShadow:'0 8px 32px rgba(138,46,136,0.5)' }}>
                  {studentData.photoUrl ? (
                    <img src={studentData.photoUrl} alt={studentData.fullName}
                      className="w-full h-full rounded-[18px] object-cover" />
                  ) : (
                    <User size={48} color="#fff" />
                  )}
                </div>
                <button className="px-4 py-2 rounded-[10px] bg-[#8A2E88]/20 border border-[#8A2E88]/35
                ...
                <>
                  ...
                  <div className="rounded-[20px] overflow-hidden border border-[#8A2E88]/18
                    ...
                    <div className="p-6 space-y-4">
                      {[
                        { icon: <User size={16} />, label: 'Father', value: studentData.fatherName },
                        { icon: <User size={16} />, label: 'Mother', value: studentData.motherName },
                        { icon: <Calendar size={16} />, label: 'Date of Birth', value: new Date(studentData.dateOfBirth).toLocaleDateString() },
                        { icon: <MapPin size={16} />, label: 'Region', value: studentData.region },
                        { icon: <Mail size={16} />, label: 'Email', value: studentData.email },
                        { icon: <Phone size={16} />, label: 'Phone', value: studentData.phoneNumber }
                      ].map((item, i) => (
                        <div key={i} className="flex items-start gap-3">
                          <span className="text-[#C084C8]/70 mt-0.5">{item.icon}</span>
                          <div className="flex-1">
                            <p className="text-[0.75rem] font-semibold text-[#967AA5]/60 uppercase tracking-[0.04em] mb-0.5">
                              {item.label}
                            </p>
                            <p className="text-[0.9rem] text-white">{item.value}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="rounded-[20px] overflow-hidden border border-[#8A2E88]/18
                    ...
                    <div className="p-6 space-y-4">
                      {[
                        ...
                        { label: 'Aadhar Number', value: studentData.aadharNumber },
                        { label: 'Joining Date', value: new Date(studentData.joiningDate).toLocaleDateString() },
                        { label: 'Graduation Date', value: new Date(studentData.graduationDate).toLocaleDateString() }
                      ].map((item, i) => (
                        <div key={i} className="flex justify-between items-center pb-3 border-b border-white/[0.06] last:border-0">
                          <span className="text-[0.85rem] text-[#C8A0D7]/60">{item.label}</span>
                          <span className="text-[0.9rem] font-medium text-white">{item.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="p-6 space-y-4">
                    {studentData.academicHistory.map((record, i) => (
                      <div key={i} className="flex gap-4 pb-4 border-b border-white/[0.06] last:border-0">
                        <div className="w-10 h-10 rounded-[10px] bg-gradient-to-br from-[#8A2E88] to-[#C084C8]
                          flex items-center justify-center text-white flex-shrink-0">
                          <BookOpen size={18} />
                        </div>
                        <div className="flex-1">
                          <p className="font-semibold text-white mb-1">{record.degree}</p>
                          <p className="text-[0.85rem] text-[#C8A0D7]/60">{record.institution}</p>
                          <div className="flex justify-between mt-2 text-[0.8rem]">
                            <span className="text-[#967AA5]/60">Completed: {record.yearOfCompletion}</span>
                            <span className="text-[#C084C8]">Grade: {record.grade}</span>
                          </div>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="p-6">
                      <div className="flex flex-wrap gap-2">
                        {studentData.skills.map((skill, i) => (
                          <span key={i} className="px-3 py-1.5 rounded-full bg-[#8A2E88]/25
                      ...
                      {studentData.certifications.map((cert, i) => (
                        <div key={i} className="flex gap-4 pb-4 border-b border-white/[0.06] last:border-0">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-[0.85rem] text-[#C8A0D7]/60">{cert.issuer}</p>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="rounded-[20px] overflow-hidden border border-[#8A2E88]/18
                ...
                <div className="p-6 space-y-3">
                  {[
                    { label: 'Certificates', value: studentData.certifications.length },
                    { label: 'Projects', value: studentData.projects.length },
                    { label: 'Skills', value: studentData.skills.length }
                  ].map((stat, i) => (
                    <div key={i} className="flex justify-between items-center">
                      <span className="text-[#C8A0D7]/60 text-[0.9rem]">{stat.label}</span>
                      <span className="font-bold text-[1.2rem] text-[#C084C8]">{stat.value}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="p-6 space-y-3">
                  {studentData.projects.map((project, i) => (
                    <div key={i} className="p-3 rounded-[12px] bg-[#8A2E88]/10 border border-[#8A2E88]/20
                      ...
                      <p className="font-medium text-[0.9rem] text-white mb-1">{project.title}</p>
                      <p className="text-[0.75rem] text-[#C8A0D7]/50 line-clamp-2">{project.description}</p>
                      <p className="text-[0.75rem] text-[#967AA5]/60 mt-2">{project.year}</p>
                    </div>
                  ))}
                </div>
              </div>
